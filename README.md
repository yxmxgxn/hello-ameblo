...
運用の引き継ぎ（いまの状態・決めごと・宿題・過去の失敗）は [HANDOFF.md](HANDOFF.md) にまとめてある。
...
3. 削除チェック: 一覧を1ページ目から順に読み直し、こちらにあるのに一覧から消えた記事を、記事ページが404（または非公開）と確かめてから削除する。1回150ページ（3000記事）ずつ、全部見終えたらまた最初から
4. 残りの時間で過去記事を1ページ（20件）ずつ、全ブログ順番に遡る

アメンバー限定の記事は取り込まない。`crawler/exclude.txt` に書いたブログは巡回せず、取り込み済みの分も消す（削除依頼への対応もここ）。
